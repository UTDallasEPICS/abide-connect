...
<template>
  <NuxtLink
    :to="url"
    class="group relative block w-64 shrink-0 origin-center transform-gpu cursor-pointer overflow-hidden rounded-2xl border-2 border-gray-300 bg-white text-left shadow-sm transition-[background-color,transform] duration-100 ease-out hover:bg-gray-50 active:scale-[0.97] active:bg-gray-100"
  >
    ...
    <div class="relative h-28 w-full overflow-hidden">
      ...
      >
        <span class="text-lg font-semibold leading-tight text-gray-900">{{ day }}</span>
      ...
      >
        <div class="flex -space-x-2">
          <div
            ...
            :key="index"
            class="h-6 w-6 shrink-0 rounded-full ring-2 ring-white transition-[--tw-ring-color] duration-100 ease-out group-hover:ring-gray-50 group-active:ring-gray-100"
          >
            <UserAvatar :name="initials" />
          </div>
        </div>
        <span class="text-xs font-normal text-blue-600">+{{ going }} Attendees</span>
      </div>
    </div>
  </NuxtLink>
</template>
